const questions = [
  ...
  for (let domanda of questions) {
    domande.push(domanda.question)
    domanda.incorrect_answers.push(domanda.correct_answer)
    risposte.push(domanda.incorrect_answers)
        
  }
  console.log(domande);
  console.log(risposte);

  let numeroCasuale = Math.floor(Math.random() * domande.length);
let rispostaCasuale = Math.floor(Math.random() * risposte[numeroCasuale].length)
    let bloccoDomande = document.createElement("div");
    let logo = document.querySelector("#logo");
    bloccoDomande.textContent = domande[numeroCasuale];
    ...
    bottone1.textContent = risposte[numeroCasuale][rispostaCasuale]
    bottone2.textContent = risposte[numeroCasuale][rispostaCasuale]
